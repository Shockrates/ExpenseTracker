...
public class ExpenseService implements IExpenseService {
    ...
    public Page<ExpenseDTO> fetchExpenseList(int page, int size) {
        return ExpenseMapper
                .toExpenseDTOPage(expenseRepository.findAllByOrderByExpenseDateDesc(PageRequest.of(page, size)));
    }
    ...
    public Optional<ExpenseDetailedDTO> fetchExpense(Long ExpenseId) {
        return expenseRepository.findById(ExpenseId)
                .map(ExpenseMapper::toDetailedDTO);
    }
    ...
    public Optional<ExpenseDTO> updateExpense(Long ExpenseId, Expense updatedExpense) {
        ...
        return expenseRepository.findById(ExpenseId).map(expense -> {
            ...
            if (updatedExpense.getExpenseUser() != null
                    && userRepository.existsById(updatedExpense.getExpenseUser().getUserId())) {
                expense.setExpenseUser(updatedExpense.getExpenseUser());
            } else
                throw new ResourceNotFoundException("User not found");

            if (updatedExpense.getExpenseCategory() != null
                    && categoryRepository.existsById(updatedExpense.getExpenseCategory().getCategoryId())) {
    ...
    public Page<ExpenseDTO> fetchExpensesByCategory(Long categoryId, int page, int size) {
        ...
        return ExpenseMapper
                .toExpenseDTOPage(expenseRepository.findByExpenseCategory(category, PageRequest.of(page, size)));
    }
